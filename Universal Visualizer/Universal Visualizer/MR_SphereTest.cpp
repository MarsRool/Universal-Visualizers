...
MR::Geometry::SphereTest::SphereTest(const MR::Model::Model &model, const Geometry::Point3DCartesian &point, double radius) : TestModel(model, false), TestPoint(point)
{
	this->Radius = radius;
	isOk = check();
	test();
